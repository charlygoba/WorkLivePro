...
<script>
    document.querySelectorAll('.toggle-key').forEach((button) => button.addEventListener('click', () => {
        ...
        button.parentElement?.querySelector('.copy-key')?.classList.toggle('hidden', revealed);
    }));
    document.querySelectorAll('.copy-key').forEach((button) => button.addEventListener('click', async () => {
        await navigator.clipboard?.writeText(button.dataset.copy);
        const icon = button.querySelector('i');
        icon.className = 'fa-solid fa-check';
        button.setAttribute('title', 'KEY copiada');
        setTimeout(() => { icon.className = 'fa-regular fa-copy'; button.setAttribute('title', 'Copiar KEY'); }, 1500);
    }));
