...
@php
    ...
    $hasFilters = collect(['date_from','date_to','time_from','time_to','app','domain','event_type','event_search'])->contains(fn($key) => filled(request($key))) || request()->boolean('blocked_only');
...
@endphp
<style>@media print{html,body{height:auto!important;min-height:0!important;overflow:visible!important;background:#fff!important}body *{visibility:hidden!important}main,.flex-1{height:auto!important;min-height:0!important;overflow:visible!important}.employee-timeline-print-area,.employee-timeline-print-area *{visibility:visible!important}aside,header,.no-print,.timeline-event-filter-actions,.timeline-event-filter-grid,nav,.pagination{display:none!important}.employee-timeline-print-area{position:absolute!important;top:0!important;left:0!important;width:100%!important;max-width:none!important;overflow:visible!important;border:0!important;box-shadow:none!important;background:#fff!important}.employee-timeline-print-area .timeline-panel-body{padding:0!important}.employee-timeline-print-area table{width:100%!important;min-width:0!important;table-layout:auto!important;border-collapse:collapse!important;font-size:9px!important}.employee-timeline-print-area thead{display:table-header-group!important}.employee-timeline-print-area tr{break-inside:avoid!important;page-break-inside:avoid!important}.employee-timeline-print-area .overflow-x-auto,.employee-timeline-print-area .overflow-hidden{overflow:visible!important}.employee-timeline-print-area a{color:#000!important;text-decoration:none!important}@page{size:landscape;margin:10mm}}</style>
<section class="employee-timeline-print-area timeline-panel overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    ...
    <div class="timeline-panel-body">
        ...
        <div class="mb-3 flex flex-wrap justify-end gap-2 no-print"><a href="{{ route('employees.events.export', array_merge(['id' => $employee->id], request()->query())) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-2 text-[10px] font-bold text-white hover:bg-indigo-500"><i class="fa-solid fa-file-csv"></i> Exportar CSV</a><a href="{{ route('employees.events.export.pdf', array_merge(['id' => $employee->id], request()->query())) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-rose-600 px-3 py-2 text-[10px] font-bold text-white hover:bg-rose-500"><i class="fa-solid fa-file-pdf"></i> PDF</a><button type="button" onclick="window.print()" class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-[10px] font-bold text-slate-600 hover:text-indigo-600"><i class="fa-solid fa-print"></i> Imprimir</button></div>
        <form id="event-filters" method="get" class="rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
            ...
            <div class="timeline-event-filter-grid grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-7">
                ...
                <label class="font-mono text-[9px] font-bold uppercase tracking-wider text-slate-400">Hasta<input id="event-date-to" type="date" name="date_to" value="{{ request('date_to') }}" class="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold normal-case tracking-normal text-slate-700 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"></label><label class="font-mono text-[9px] font-bold uppercase tracking-wider text-slate-400">Hora desde<input type="time" name="time_from" value="{{ $timelineTimeFrom }}" class="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold normal-case tracking-normal text-slate-700"></label><label class="font-mono text-[9px] font-bold uppercase tracking-wider text-slate-400">Hora hasta<input type="time" name="time_to" value="{{ $timelineTimeTo }}" class="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold normal-case tracking-normal text-slate-700"></label>
                <label class="font-mono text-[9px] font-bold uppercase tracking-wider text-slate-400">Estado<select name="event_type" class="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold normal-case tracking-normal text-slate-700 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"><option value="all">Todos</option>@foreach(['active'=>'Activo','idle'=>'Inactivo','suspended'=>'Suspendido','locked'=>'Bloqueado','startup'=>'Inicio','shutdown'=>'Apagado','blocked'=>'Sitio bloqueado'] as $value=>$label)<option value="{{ $value }}" @selected(request('event_type')===$value)>{{ $label }}</option>@endforeach</select></label>
